...
if diagramm == 0:
    relative = True
    if relative:
        xs = np.linspace(0, 750000, 5000) 
    else:
        xs = np.linspace(450000, 1000000, 5000)

    ys = [custom_sigmoid(x, relative) for x in xs]

    plt.plot(xs, ys)
    if relative:
        plt.title("Custom Sigmoid: center=275000, range 0.85 → 1.9")
    else:
