    ...
    for (root, dirs, files) in os.walk(f"{local_folder}/valid"):
        ...
        for i in use.get(root.split("/")[-1], []):
            ...
            if len(splits) > 1:
                ...
                print(f'    antwort{index}{i} = st.radio("Unterstützt diese Beschreibung das Thema?", ["Ja", "Nein"], key="antwort{index}{i}")', file=f)
                print(f'    st.session_state.auswahl["Seite {index} Frage"] = antwort{index}{i}', file=f)
            print("", file=f)
            ...
            if len(tmp_ret) >= 1:
                ...
                print(f'        index=st.session_state.auswahl.get("Seite {index} Valides Bild", 0),', file=f)
                print(f'        key=f"frage{index}{i}_{{st.session_state.reload_counter}}"', file=f)
                print('    )', file=f)
                print(f'    st.session_state.auswahl["Seite {index} Valides Bild"] = img{index}{i}', file=f)
                print(f'    valid{index}{i}_tmp = st.session_state.reload_counter % 2', file=f)
                print(f'    if valid{index}{i}_tmp == 0:', file=f)
                print(f'        st.image(tmp{index}{i}[st.session_state.auswahl.get("Seite {index} Valides Bild", 0)] if valid{index}{i}_tmp == 0 else "{image_link}")', file=f)
                print(f'    else:', file=f)
                print(f'        st.image(tmp{index}{i}[st.session_state.auswahl.get("Seite {index} Valides Bild", 0)] if valid{index}{i}_tmp == 1 else "{image_link}")', file=f)
                print(f'    st.text_input("Gewähltes Bild", value=st.session_state.auswahl["Seite {index} Valides Bild"], key="wv{index}{i}", disabled=True)', file=f)
            ...
            if len(tmp_not_valid_ret) >= 1:
                ...
                print(f'        nvtmp{index}{i},', file=f)
                print('        return_value="index",', file=f)
                print(f'        index=st.session_state.auswahl.get("Seite {index} Nicht Valides Bild", 0),', file=f)
                ...
                print(f'    st.session_state.auswahl["Seite {index} Nicht Valides Bild"] = nvimg{index}{i}', file=f)
                print(f'    nvalid{index}{i}_tmp = st.session_state.reload_counter % 2', file=f)
                print(f'    if nvalid{index}{i}_tmp == 0:', file=f)
                print(f'        st.image(nvtmp{index}{i}[st.session_state.auswahl.get("Seite {index} Nicht Valides Bild", 0)] if nvalid{index}{i}_tmp == 0 else "{image_link}")', file=f)
                print(f'    else:', file=f)
                print(f'        st.image(nvtmp{index}{i}[st.session_state.auswahl.get("Seite {index} Nicht Valides Bild", 0)] if nvalid{index}{i}_tmp == 1 else "{image_link}")', file=f)
                print(f'    st.text_input("Gewähltes Bild: ", value=st.session_state.auswahl["Seite {index} Nicht Valides Bild"], key="nvw{index}{i}", disabled=True)', file=f)
                print("", file=f)
